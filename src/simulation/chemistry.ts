...
export const bondKindLabel: Record<BondKind, string> = {
...
export function desiredBondOrder(a: AtomSymbol, b: AtomSymbol): 1 | 2 | 3 {
  return covalentPairs[pairKey(a, b)] ?? 1;
}

export function classifyBond(a: AtomSymbol, b: AtomSymbol, emphasis: number): BondKind {
  const atomA = atomData[a];
  const atomB = atomData[b];
  if (atomA.metal && atomB.metal) return "metallic";
  const diff = Math.abs(atomA.electronegativity - atomB.electronegativity) * emphasis;
  if (diff >= 1.7 && (atomA.metal || atomB.metal)) return "ionic";
  if (diff >= 0.45) return "polar-covalent";
  return "nonpolar-covalent";
}

export function availableBondSlots(atom: AtomParticle, bonds: Bond[]) {
...
export function canBond(a: AtomParticle, b: AtomParticle, bonds: Bond[], settings: SimulationSettings) {
  if (a.id === b.id) return false;
  if (bonds.some((bond) => (bond.a === a.id && bond.b === b.id) || (bond.a === b.id && bond.b === a.id))) return false;
  const kind = classifyBond(a.symbol, b.symbol, settings.electronegativityEmphasis);
  if (kind === "metallic") return false;
...
export function makeBond(a: AtomParticle, b: AtomParticle, time: number, settings: SimulationSettings): Bond {
  const kind = classifyBond(a.symbol, b.symbol, settings.electronegativityEmphasis);
  const order = kind === "ionic" ? 1 : desiredBondOrder(a.symbol, b.symbol);
...
export function applyIonCharges(a: AtomParticle, b: AtomParticle) {
  const atomA = atomData[a.symbol];
  const atomB = atomData[b.symbol];
...
export function eventForBond(bond: Bond, a: AtomParticle, b: AtomParticle): BondEvent {
  const atomA = atomData[a.symbol];
  const atomB = atomData[b.symbol];
...
export function stableShellText(symbol: AtomSymbol) {
  const atom = atomData[symbol];
  if (symbol === "H") return "duet tendency: 2 electrons in the first shell";
  if (atom.nobleGas) return "already stable: full valence shell";
  if (atom.metal) return "often stabilizes by losing valence electrons";
