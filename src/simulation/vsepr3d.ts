...
export function applyVsepr3DTargets(atoms: AtomParticle[], bonds: Bond[]) {
  const graph = buildMoleculeGraph(atoms, bonds);
  const targets = new Map<string, { x: number; y: number; z: number; count: number }>();
  ...
  for (const center of atoms) {
    ...
    if (!analysis || neighbors.length < 2) continue;
    const baseVectors = templates[analysis.axe] ?? templates[`AX${neighbors.length}`];
    if (!baseVectors || baseVectors.length < neighbors.length) continue;
