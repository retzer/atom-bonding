...
import { buildMoleculeGraph, structuralBonds } from "../simulation/graph.js";
...
const defaultCamera = { yaw: -0.62, pitch: 0.46, panX: 0, panY: 0, fitZoom: 1 };
const templates = {
...
export function Molecule3DView({ state, settings, running, width, height, onResize, onSelectAtom, onZoom, onToggle3D }) {
    ...
    const sceneAtoms = useMemo(() => buildSceneAtoms(state.atoms, state.bonds, settings, state.time), [settings.collisionStrength, settings.geometryAssist, settings.geometryMode, settings.relaxationStrength, settings.speed, settings.temperature, state.atoms, state.bonds, state.time]);
    const sceneKey = useMemo(() => `${state.atoms.map((atom) => atom.id).join("|")}::${state.bonds.map((bond) => bond.id).join("|")}`, [state.atoms, state.bonds]);
    useEffect(() => {
        if (!wrapRef.current)
            return;
    ...
    const setZoom = (zoom) => onZoom(clamp(zoom, 0.55, 2.2));
    ...
    return (_jsxs("div", { className: "simulation-wrap molecule-3d-wrap", ref: wrapRef, children: [_jsx("canvas", { ref: webglCanvasRef, className: "simulation-canvas molecule-3d-webgl", "aria-hidden": "true" }), _jsx("canvas", { ref: canvasRef, className: "simulation-canvas molecule-3d-canvas molecule-3d-overlay", "aria-label": "3D VSEPR molecule renderer", title: "Drag to rotate. Shift-drag or right-drag to pan.", onPointerDown: onPointerDown, onPointerMove: onPointerMove, onPointerUp: onPointerUp, onPointerCancel: onPointerUp, onPointerLeave: () => {
                    setHoveredAtomId(null);
                    setHoveredBondId(null);
                }, onContextMenu: (event) => event.preventDefault() }), _jsxs("div", { className: "canvas-readout", children: [_jsxs("span", { children: [_jsx(Box, { size: 13 }), " VSEPR 3D renderer"] }), _jsxs("span", { children: [Math.round(camera.fitZoom * 100), "% fit"] }), _jsxs("span", { children: [state.atoms.length, " atoms"] }), _jsxs("span", { children: [state.bonds.length, " bonds"] })] }), !state.atoms.length && (_jsxs("div", { className: "empty-canvas-hint", children: [_jsx("strong", { children: "No molecule to render" }), _jsx("span", { children: "Build or choose a molecule, then drag to rotate or Shift-drag to pan." })] }))] }));
}
function buildSceneAtoms(atoms, bonds, settings, time) {
    if (!atoms.length)
        return [];
...
function apply3DControlResponse(placed, atoms, bonds, settings, time) {
    if (settings.geometryMode === "rigid")
        return;
    if (atoms.length > 64)
        return;
...
function normalizedLivePoints(atoms) {
...
function applySceneRepulsion(placed, atoms, bonds, strength) {
    if (strength <= 0 || atoms.length < 2 || atoms.length > 220)
        return;
    ...
    for (let i = 0; i < atoms.length; i += 1) {
        for (let j = i + 1; j < atoms.length; j += 1) {
            const a = atoms[i];
            const b = atoms[j];
            if (bonded.has([a.id, b.id].sort().join("|")))
                continue;
...
function placeVseprComponent(component, bonds, graph, placed, offset) {
    ...
    for (const atom of component) {
        if (!placed.has(atom.id))
            placed.set(atom.id, { x: offset.x + (atom.x % 80) / 90, y: (atom.y % 80) / 90, z: 0 });
    }
}
function placeFlatComponent(component, placed, offset) {
...
function largeMoleculeTargetSpan(atomCount) {
    if (atomCount > 120)
        return clamp(Math.sqrt(atomCount) * 1.36, 14, 22);
    if (atomCount > 72)
        return clamp(Math.sqrt(atomCount) * 1.24, 11, 17);
    if (atomCount > 36)
        return clamp(Math.sqrt(atomCount) * 1.04, 7.2, 12);
    return 5.2;
}
...
function draw3D(ctx, atoms, sourceAtoms, bonds, settings, camera, width, height, hoveredAtomId, hoveredBondId, longHover, time, selectedAtomId) {
    ...
    const surfacePorts = settings.showElectronRegions ? buildSurfacePorts(projected, visibleBonds, settings, interaction, lighting) : new Map();
    ...
    if (showChemistryDetail && settings.showElectronRegions && budget.overlays)
        drawElectronRegionOverlay(ctx, projected, bonds, settings, false);
    if (shouldShowFunctionalGroups(settings, detailLevel))
        drawFunctionalGroupHighlights3D(ctx, projected, structuralBonds(bonds), settings, detailLevel);
    for (const bond of [...visibleBonds].sort((a, b) => bondDepth(a, byId) - bondDepth(b, byId))) {
        const a = byId.get(bond.a);
        const b = byId.get(bond.b);
        if (a && b)
            draw3DBond(ctx, a, b, bond, settings, budget, interaction, lighting);
    }
    if (showChemistryDetail && (settings.showBondDipoles || settings.showNetDipole) && budget.overlays) {
        if (settings.showBondDipoles)
            for (const bond of [...visibleBonds].sort((a, b) => bondDepth(a, byId) - bondDepth(b, byId))) {
                const a = byId.get(bond.a);
                const b = byId.get(bond.b);
                if (a && b)
                    draw3DDipoleIndicator(ctx, a, b, bond, settings, interaction);
            }
        if (settings.showNetDipole)
    ...
    if (showChemistryDetail && settings.showElectronFlow && budget.overlays)
        drawElectronFlow3D(ctx, projected, visibleBonds.filter((bond) => bond.id !== hoveredBondId), settings, time);
    if (budget.overlays)
        draw3DBondAngleBadge(ctx, projected, visibleBonds, hoveredBondId, settings);
    drawAxisGizmo(ctx, camera, settings, width, height);
    drawMini2DStructure(ctx, sourceAtoms, bonds, settings);
}
function draw3DOverlay(ctx, atoms, sourceAtoms, bonds, settings, camera, width, height, hoveredAtomId, hoveredBondId, longHover, time, selectedAtomId) {
    ctx.clearRect(0, 0, width, height);
    const lighting = sceneLighting(settings);
    const detailLevel = detailLevelFor(settings);
    const interaction = buildInteractionContext(sourceAtoms, bonds, settings, selectedAtomId, hoveredAtomId, hoveredBondId, longHover);
    ...
    draw3DLightSource(ctx, lighting, settings, width);
    if (showChemistryDetail && settings.showElectronRegions && budget.overlays)
        drawElectronRegionOverlay(ctx, projected, bonds, settings);
    if (shouldShowFunctionalGroups(settings, detailLevel))
        drawFunctionalGroupHighlights3D(ctx, projected, structuralBonds(bonds), settings, detailLevel);
...
function draw3DAtomLabels(ctx, atoms, bonds, settings, budget, interaction) {
    ...
    for (const atom of [...visibleAtoms].sort((a, b) => a.depth - b.depth)) {
        if (!projectedAtomOverlayVisible(atom, visibleAtoms))
            continue;
...
function projectedAtomOverlayVisible(atom, atoms) {
    for (const other of atoms) {
        if (other.id === atom.id)
            continue;
        if (other.depth <= atom.depth + 0.06)
            continue;
        const distance = Math.hypot(atom.sx - other.sx, atom.sy - other.sy);
        if (distance < other.screenRadius * 0.82)
            return false;
    }
    return true;
}
function modelWorldRadiusFor(atom, sceneSize = 1) {
...
function largeMoleculeAtomScale(atomCount) {
    if (atomCount > 150)
        return 0.52;
    if (atomCount > 96)
        return 0.58;
...
function renderWebGL3D(canvas, rendererRef, atoms, sourceAtoms, bonds, settings, camera, width, height, hoveredAtomId, hoveredBondId, longHover, time, selectedAtomId, dpr) {
    ...
    gl.uniform1f(renderer.locations.aoStrength, aoStrengthForQuality(settings.graphicsQuality));
...
function createWebGLRenderer(canvas, quality) {
    ...
    return {
        ...
        locations: {
            ...
            aoStrength: gl.getUniformLocation(program, "uAoStrength")
        },
...
function drawWebGLBonds(renderer, atoms, bonds, worldRadii, settings, camera, interaction) {
    ...
    for (const bond of bonds) {
        ...
        const hasHydrogen = a.symbol === "H" || b.symbol === "H";
        const baseTube = clamp((0.045 - (bond.order - 1) * 0.005) * (hasHydrogen ? 1.22 : 1), 0.032, 0.058);
        ...
        const perspectiveBoost = settings.projectionMode === "deep-perspective" ? 0.003 : settings.projectionMode === "soft-perspective" ? 0.001 : 0;
        const depthOffset = 0.005 + perspectiveBoost;
...
function drawWebGLGroundShadow(renderer, atoms, bonds, worldRadii, settings, lighting, budget) {
    if (!atoms.length || settings.displayMode === "skeleton")
        return;
    const visibleAtoms = atoms.filter((atom) => atomVisibleInMode(atom, settings));
    ...
    for (const bond of bonds) {
        const a = byId.get(bond.a);
        const b = byId.get(bond.b);
        if (!a || !b || !atomVisibleInMode(a, settings) || !atomVisibleInMode(b, settings))
            continue;
    ...
    for (const atom of visibleAtoms) {
        ...
        if (softPass) {
            drawWebGLMesh(renderer, renderer.sphereLow, basisMatrix(center, { x: radius * 1.95, y: 0, z: 0 }, { x: 0, y: 0.012, z: 0 }, { x: 0, y: 0, z: radius * 0.78 }), shadowColor, baseAlpha * 0.16, 0.13);
        }
        drawWebGLMesh(renderer, renderer.sphereLow, basisMatrix(center, { x: radius * 1.34, y: 0, z: 0 }, { x: 0, y: 0.018, z: 0 }, { x: 0, y: 0, z: radius * 0.5 }), shadowColor, baseAlpha * 0.42, 0.11);
    }
}
function bondAtomTubeColor(atom, settings) {
...
function drawWebGLAtoms(renderer, atoms, bonds, worldRadii, settings, interaction, budget) {
...
function drawWebGLSurfaceSockets(renderer, atoms, bonds, worldRadii, settings, camera, interaction) {
...
function drawWebGLSocketSet(renderer, atoms, atom, other, bond, worldRadii, color, view, settings, interaction) {
    ...
    if (other.depth < atom.depth - 0.26)
        return;
...
function drawWebGLLonePairs(renderer, sceneAtoms, projectedAtoms, bonds, worldRadii, settings, time, budget) {
    if (settings.displayMode === "skeleton" || !budget.overlays && settings.analysisMode !== "chemistry")
        return;
    const detailLevel = detailLevelFor(settings);
    ...
    for (const cloud of clouds) {
        ...
        if (!center || !atomVisibleInMode(center, settings))
            continue;
        if (!projectedAtomOverlayVisible(center, visibleProjectedAtoms))
            continue;
...
function webGLWorldRadius(atom) {
    return modelWorldRadiusFor(atom, atom.sceneSize);
}
function drawWebGLMesh(renderer, mesh, model, color, alpha, depthOffset, vertexPerspective = false) {
    const gl = renderer.gl;
...
function createWebGLMesh(gl, geometry) {
...
function createSphereGeometry(rows, columns) {
    const positions = [];
    const normals = [];
    const indices = [];
...
function createLonePairGeometry(rows, columns) {
    const positions = [];
    const normals = [];
    const indices = [];
    for (let y = 0; y <= rows; y += 1) {
...
void main() {
  ...
  float depthAO = 1.0 - uAoStrength * clamp(0.5 - vDepth * 0.08, 0.0, 0.5);
  float cavity = 1.0 - uAoStrength * 0.28 * max(0.0, 1.0 - abs(dot(normal, viewNormal)));
  float curvature = 1.0 - uAoStrength * 0.12 * max(0.0, -dot(normal, viewNormal));
  float ao = depthAO * cavity;
  ...
  float rimPower = 2.45 + (1.0 - uAoStrength) * 1.5;
  float rim = pow(1.0 - max(dot(viewNormal, vec3(0.0, 0.0, 1.0)), 0.0), rimPower);
  float terminator = smoothstep(-0.22, 0.54, dot(normal, light));
  vec3 shadedBase = mix(uColor * (0.58 + uAoStrength * 0.08), uColor * curvature, terminator);
  vec3 color = shadedBase * (uAmbient * ao + uDiffuse * wrappedDiffuse) + uLightColor * (uSpecular * (specular + broadSpecular) + uRim * rim * ao);
  gl_FragColor = vec4(clamp(color, 0.0, 1.0), uAlpha);
}
`;
function projectAtoms(atoms, camera, width, height, settings) {
...
function projectLonePairs(clouds, camera, width, height, settings) {
    return clouds.map((cloud) => ({ ...cloud, ...projectPoint(cloud, cloud.radius, camera, width, height, settings) }));
}
function projectPoint(point, radius, camera, width, height, settings, minScreenRadius = 8) {
    const profile = projectionProfile(settings.projectionMode);
    const totalZoom = settings.zoom * camera.fitZoom;
...
function renderBudgetFor(atoms, settings) {
...
function aoStrengthForQuality(quality) {
    if (quality === "low")
        return 0;
    if (quality === "medium")
        return 0.35;
...
function detailLevelFor(settings) {
...
function shouldShowFunctionalGroups(settings, detailLevel) {
    return settings.showFunctionalGroups || settings.analysisMode === "structure" && detailLevel !== "detail";
}
function atomVisibleInMode(atom, settings) {
    const detailLevel = detailLevelFor(settings);
    if (settings.displayMode === "skeleton")
        return false;
    if (settings.displayMode === "simplified" && atom.symbol === "H")
...
function shouldShowAtomLabel(atom, settings, budget, focusIds) {
    const detailLevel = detailLevelFor(settings);
    if (!settings.showLabels || !budget.labels || settings.displayMode === "skeleton")
        return false;
    if (settings.analysisMode === "structure" && detailLevel !== "detail")
        return focusIds?.has(atom.id) || atom.symbol !== "H" && atom.screenRadius > 34;
    if (detailLevel === "abstract")
        return focusIds?.has(atom.id) || atom.symbol !== "H" && atom.screenRadius > 34;
    if (detailLevel === "structure")
        return focusIds?.has(atom.id) || atom.symbol !== "H";
    if (focusIds?.has(atom.id))
        return true;
    if (settings.displayMode === "simplified" && atom.symbol === "H")
        return false;
    if (settings.zoom < 0.8)
        return atom.symbol !== "H" && atom.screenRadius > 24;
    if (settings.zoom < 1.15)
        return atom.symbol !== "H";
    return true;
}
function bondVisibleInMode(bond, atoms, settings) {
    const detailLevel = detailLevelFor(settings);
    if (settings.displayMode === "skeleton")
        return true;
...
function focusAtomIds(atoms, bonds, settings, selectedAtomId) {
    if (!settings.focusMode || !selectedAtomId || !atoms.some((atom) => atom.id === selectedAtomId))
        return null;
    const ids = new Set([selectedAtomId]);
    for (const bond of structuralBonds(bonds)) {
        if (bond.a === selectedAtomId)
            ids.add(bond.b);
        if (bond.b === selectedAtomId)
            ids.add(bond.a);
    }
    return ids;
}
function buildInteractionContext(atoms, bonds, settings, selectedAtomId, hoveredAtomId, hoveredBondId, longHover) {
    const graph = buildMoleculeGraph(atoms, bonds);
    const centralAtomIds = new Set();
    for (const atom of atoms) {
        const degree = graph.neighborsById.get(atom.id)?.length ?? 0;
        const analysis = analyzeAtomGeometry(atom, atoms, bonds);
        if (atom.symbol !== "H" && degree >= 2 && analysis?.bondedAtoms)
            centralAtomIds.add(atom.id);
    }
    const activeAtomIds = new Set();
    const addAtomAndNeighbors = (id) => {
        if (!id)
            return;
        activeAtomIds.add(id);
        for (const neighbor of graph.neighborsById.get(id) ?? [])
            activeAtomIds.add(neighbor.id);
    };
    ...
    for (const bond of structuralBonds(bonds)) {
        if (bond.id === hoveredBondId || activeAtomIds.has(bond.a) && activeAtomIds.has(bond.b))
...
function applyProjectedEmphasis(atoms, interaction, settings) {
    if (settings.displayMode === "skeleton")
        return atoms;
...
function emphasisScaleForAtom(atom, interaction) {
    let scale = 1;
    if (interaction.centralAtomIds.has(atom.id))
        scale += atom.symbol === "H" ? 0 : 0.055;
    if (interaction.activeAtomIds.has(atom.id))
        scale += 0.035;
    if (interaction.selectedAtomId === atom.id)
        scale += 0.12;
    if (interaction.hoveredAtomId === atom.id)
        scale += 0.065 + interaction.longHover * 0.07;
    return scale;
}
function drawDepthSortedStructure(ctx, atoms, lonePairs, bonds, settings, budget, interaction, time, byId, surfacePorts, lighting) {
    const visibleAtoms = atoms.filter((atom) => atomVisibleInMode(atom, settings));
    const atomIds = new Set(visibleAtoms.map((atom) => atom.id));
    const items = [
        ...visibleAtoms.map((atom) => ({ kind: "atom", id: atom.id, depth: atomPaintDepth(atom, atoms, bonds), atom })),
        ...lonePairs
            .filter((cloud) => atomIds.has(cloud.centerId))
            .map((cloud) => ({ kind: "lonePair", id: cloud.id, depth: lonePairPaintDepth(cloud, byId.get(cloud.centerId)), cloud }))
    ...
    for (const item of items) {
        ...
        if (center)
            drawLonePairCloud(ctx, item.cloud, center, settings, time, lighting);
    }
}
function lonePairPaintDepth(cloud, center) {
...
function frontBondedAtoms(atoms, bonds) {
    const byId = new Map(atoms.map((atom) => [atom.id, atom]));
    const front = new Map();
    for (const bond of bonds) {
        const a = byId.get(bond.a);
        ...
        if (overlap <= smaller.screenRadius * 0.08)
            continue;
        if (smaller.depth < larger.depth - 0.75)
            continue;
        front.set(smaller.id, smaller);
    }
    return [...front.values()].sort((a, b) => atomPaintDepth(a, atoms, bonds) - atomPaintDepth(b, atoms, bonds));
}
function lonePairClouds(atoms, bonds) {
    const clouds = [];
    const graph = buildMoleculeGraph(atoms, bonds);
    const componentSize = componentSizesByAtom(atoms, structuralBonds(bonds));
    for (const atom of atoms) {
        const analysis = analyzeAtomGeometry(atom, atoms, bonds);
        if (!analysis?.lonePairs)
            continue;
        if ((componentSize.get(atom.id) ?? atoms.length) > 80)
            continue;
        const neighbors = graph.neighborsById.get(atom.id) ?? [];
        const vectors = lonePairDirectionsFor(atom, neighbors, analysis.lonePairs);
...
function componentSizesByAtom(atoms, bonds) {
    const graph = buildMoleculeGraph(atoms, bonds);
    const sizes = new Map();
    const seen = new Set();
    for (const atom of atoms) {
        ...
        for (const id of ids)
            sizes.set(id, ids.length);
    }
    return sizes;
}
function lonePairDirectionsFor(atom, neighbors, lonePairs) {
    if (lonePairs <= 0)
        return [];
...
function frameCameraFor(atoms, camera, settings, width, height) {
    if (!atoms.length)
        return { ...camera, panX: 0, panY: 0, fitZoom: 1 };
...
function cameraFrameFor(atoms, camera, settings, width, height) {
    ...
    if (!bounds)
        return { panX: 0, panY: 0, fitZoom: 1 };
...
function projectedBounds(atoms) {
    if (!atoms.length)
        return null;
...
function projectionProfile(mode) {
    if (mode === "orthographic") {
        return { scale: 0.19, depthBoost: 1, distance: Infinity, minPerspective: 1, maxPerspective: 1, scaleAtomRadius: false };
    }
    if (mode === "deep-perspective") {
        return { scale: 0.2, depthBoost: 1.55, distance: 5.5, minPerspective: 0.58, maxPerspective: 1.8, scaleAtomRadius: true };
    }
    return { scale: 0.185, depthBoost: 1.18, distance: 7.2, minPerspective: 0.72, maxPerspective: 1.38, scaleAtomRadius: true };
}
...
function stablePerpendicular(direction, reference) {
    let tangent = crossVec(direction, reference);
    if (Math.hypot(tangent.x, tangent.y, tangent.z) < 0.001)
        tangent = crossVec(direction, { x: 0, y: 1, z: 0 });
    if (Math.hypot(tangent.x, tangent.y, tangent.z) < 0.001)
        tangent = crossVec(direction, { x: 1, y: 0, z: 0 });
...
function basisMatrix(origin, xAxis, yAxis, zAxis) {
    return [
...
function sphereMatrix(center, radius) {
...
function draw3DBackground(ctx, settings, width, height, lighting) {
...
function draw3DFadingFloor(ctx, settings, width, height, lighting) {
...
function draw3DMoleculeShadow(ctx, atoms, bonds, settings, camera, width, height, budget, lighting) {
    if (!atoms.length || settings.visualStyle !== "detailed")
        return;
...
function projectWorldShadowAtoms(atoms, camera, width, height, settings, lighting) {
...
function drawAggregateMoleculeShadow(ctx, atoms, settings, budget, lighting) {
...
function drawLonePairCloud(ctx, cloud, center, settings, time, lighting) {
    ...
    ctx.bezierCurveTo(baseX + px * neckWidth, baseY + py * neckWidth, bulbX + px * lobeWidth, bulbY + py * lobeWidth, bulbX + ux * r * 0.04, bulbY + uy * r * 0.04);
    ctx.bezierCurveTo(bulbX - px * lobeWidth, bulbY - py * lobeWidth, baseX - px * neckWidth, baseY - py * neckWidth, tipX, tipY);
    ctx.closePath();
    ctx.fill();
    ctx.shadowBlur = 0;
...
function drawBlinkingElectron(ctx, x, y, radius, blink, alpha) {
    ctx.save();
    ctx.globalAlpha = alpha;
...
function drawElectronRegionOverlay(ctx, atoms, bonds, settings, drawBondRegions = true) {
    const byId = new Map(atoms.map((atom) => [atom.id, atom]));
    ctx.save();
    for (const atom of atoms) {
        const analysis = analyzeAtomGeometry(atom, atoms, bonds);
        if (!analysis || analysis.electronDomains < 2)
            continue;
...
function drawFunctionalGroupHighlights3D(ctx, atoms, bonds, settings, detailLevel) {
    if (!atoms.length || settings.displayMode === "skeleton")
        return;
    const byId = new Map(atoms.map((atom) => [atom.id, atom]));
    ...
    for (const group of groups) {
        const pts = group.atomIds.map((id) => byId.get(id)).filter((atom) => Boolean(atom));
        if (!pts.length)
            continue;
...
function draw3DBond(ctx, a, b, bond, settings, budget, interaction, lighting) {
    ...
    const perspectiveLineBoost = settings.projectionMode === "deep-perspective" ? 1.15 : settings.projectionMode === "soft-perspective" ? 1.08 : 1;
    const lineWidth = clamp((a.screenRadius + b.screenRadius) * 0.075 * (1 + avgDepth * 0.08 + lighting.power * 0.08) * (1 + highlight * 0.2) * perspectiveLineBoost, 2.8, 13);
    ...
    for (const offset of offsets) {
        ...
        const tubeGradient = ctx.createLinearGradient(startX - segment.px * lineWidth * 0.58 * lightOffset, startY - segment.py * lineWidth * 0.58 * lightOffset, startX + segment.px * lineWidth * 0.58 * lightOffset, startY + segment.py * lineWidth * 0.58 * lightOffset);
...
function bondRenderColor(bond, settings) {
...
function buildSurfacePorts(atoms, bonds, settings, interaction, lighting) {
    const ports = new Map();
    if (settings.displayMode === "skeleton" || settings.zoom < 0.72)
        return ports;
    const byId = new Map(atoms.map((atom) => [atom.id, atom]));
    for (const bond of bonds) {
        const a = byId.get(bond.a);
...
function collectSurfacePorts(ports, atoms, atom, other, bond, color, settings, interaction, lighting) {
    if (settings.displayMode === "simplified" && atom.symbol === "H")
        return;
    ...
    for (const lane of lanes) {
        ...
        if (!surfacePortIsVisible(atom, other, atoms, x, y, surfaceDepth))
            continue;
...
function surfacePortIsVisible(endpoint, other, atoms, x, y, surfaceDepth) {
    if (other.depth < endpoint.depth - 0.16)
        return false;
    for (const atom of atoms) {
        if (atom.id === endpoint.id)
            continue;
        ...
        if (atomSurfaceDepth >= surfaceDepth - 0.04)
            return false;
    }
    return true;
}
function draw3DBondEndpointCaps(ctx, atoms, bonds, settings, focusIds) {
    if (settings.displayMode === "skeleton" || settings.zoom < 0.72)
        return;
    const byId = new Map(atoms.map((atom) => [atom.id, atom]));
...
function drawEndpointCap(ctx, atoms, atom, other, bond, color, settings, focusIds) {
    if (settings.displayMode === "simplified" && atom.symbol === "H")
        return;
    ...
    for (const lane of lanes) {
        ...
        const x = atom.sx + ux * atom.screenRadius * 0.9 + px * laneOffset;
        const y = atom.sy + uy * atom.screenRadius * 0.9 + py * laneOffset;
        if (!endpointCapIsVisible(atom, other, atoms, x, y))
            continue;
...
function endpointCapIsVisible(endpoint, other, atoms, x, y) {
    if (other.depth < endpoint.depth - 1.1)
        return false;
    const overlap = endpoint.screenRadius + other.screenRadius - Math.hypot(endpoint.sx - other.sx, endpoint.sy - other.sy);
    if (overlap > endpoint.screenRadius * 0.48 && other.depth < endpoint.depth + 0.08)
        return false;
    for (const atom of atoms) {
        if (atom.id === endpoint.id)
            continue;
...
function draw3DDipoleIndicator(ctx, a, b, bond, settings, interaction) {
    ...
    if (settings.showCharges) {
        ...
        drawChargeLabel3D(ctx, "δ-", target.sx + dx * (target.screenRadius + 12) + segment.px * 9, target.sy + dy * (target.screenRadius + 12) + segment.py * 9, settings.theme === "light" ? "#2563eb" : "#38bdf8", settings);
        drawChargeLabel3D(ctx, "δ+", source.sx - dx * (source.screenRadius + 12) - segment.px * 9, source.sy - dy * (source.screenRadius + 12) - segment.py * 9, settings.theme === "light" ? "#dc2626" : "#fb7185", settings);
    }
    ctx.restore();
}
function drawChargeLabel3D(ctx, text, x, y, color, settings) {
    ctx.save();
...
function drawNetDipole3D(ctx, atoms, bonds, settings) {
    const byId = new Map(atoms.map((atom) => [atom.id, atom]));
...
function drawElectronFlow3D(ctx, atoms, bonds, settings, time) {
    const byId = new Map(atoms.map((atom) => [atom.id, atom]));
    ctx.save();
    for (const bond of bonds) {
...
function visibleBondSegment(a, b) {
    ...
    const px = -uy;
    const py = ux;
    const minVisible = clamp(Math.min(a.screenRadius, b.screenRadius) * 0.18, 5, 14);
    const nominalStartInset = a.screenRadius * 0.06;
    const nominalEndInset = b.screenRadius * 0.06;
...
function draw3DAtom(ctx, atom, settings, budget, interaction, ports, lighting, overlayPass = false) {
    const data = atomData[atom.symbol];
...
function drawSurfacePortsOnAtom(ctx, atom, ports, settings, lighting) {
    ctx.save();
    ctx.beginPath();
    ...
    for (const port of ports.sort((a, b) => a.alpha - b.alpha)) {
        if (port.alpha <= 0.02)
            continue;
        const socketGradient = ctx.createRadialGradient(port.x - lighting.screen.x * port.rx * 0.55, port.y - lighting.screen.y * port.rx * 0.55, 1, port.x, port.y, port.rx * 1.42);
        ...
        ctx.beginPath();
        ctx.ellipse(port.x - lighting.screen.x * port.rx * 0.24, port.y - lighting.screen.y * port.ry * 0.4, port.rx * 1.18, port.ry * 1.26, port.rotation, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.ellipse(port.x - lighting.screen.x * port.rx * 0.28, port.y - lighting.screen.y * port.ry * 0.34, Math.max(1.2, port.rx * 0.22), Math.max(0.8, port.ry * 0.22), port.rotation, 0, Math.PI * 2);
        ctx.fill();
    }
    ctx.restore();
}
function drawAtomRadialGlow(ctx, atom, glow, settings, alpha, emphasis, lighting) {
    const r = atom.screenRadius;
    const lightPower = lighting.power;
...
function drawAtomEmphasisRing(ctx, atom, settings, state) {
    if (state.muted)
        return;
...
function draw3DBondAngleBadge(ctx, atoms, bonds, bondId, settings) {
    if (!bondId)
        return;
...
function angleBetween3D(center, a, b) {
...
function drawAxisGizmo(ctx, camera, settings, width, height) {
...
function drawMini2DStructure(ctx, atoms, bonds, settings) {
    if (!atoms.length)
        return;
...
function nearestProjectedAtom(atoms, camera, width, height, settings, x, y) {
...
function nearestProjectedBond(atoms, bonds, camera, width, height, settings, x, y) {
...
function bondDepth(bond, atoms) {
    return ((atoms.get(bond.a)?.depth ?? 0) + (atoms.get(bond.b)?.depth ?? 0)) / 2;
}
function sceneLighting(settings) {
...
function draw3DLightSource(ctx, lighting, settings, width) {
    if (settings.displayMode === "skeleton" || settings.visualStyle !== "detailed")
        return;
