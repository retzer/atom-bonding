...
import { moleculePresets, presetById } from "../data/presets.js";
import { classifyBond, eventForBond, makeBond } from "../simulation/chemistry.js";
import { createAtom, createFreeState, createPresetState, createSpawnedAtoms, stepSimulation } from "../simulation/engine.js";
const defaultSize = { width: 920, height: 640 };
const lessonSceneOffset = (width, height) => ({
...
export function useSimulation() {
    ...
    const [glucoseAnomer, setGlucoseAnomer] = useState("beta");
    const [glucoseStage, setGlucoseStage] = useState("idle");
    const [lessonAnnotations, setLessonAnnotations] = useState([]);
    const [highlightedAtomIds, setHighlightedAtomIds] = useState([]);
    const [highlightedBondIds, setHighlightedBondIds] = useState([]);
    const [currentStepIndex, setCurrentStepIndex] = useState(0);
    const [lessonStepText, setLessonStepText] = useState(undefined);
    const [lessonStepIndex, setLessonStepIndex] = useState(0);
    const [lessonTotalSteps, setLessonTotalSteps] = useState(0);
    const [animParts, setAnimParts] = useState([]);
    const [revealedCount, setRevealedCount] = useState(0);
    const [state, setState] = useState(() => createFreeState(defaultSize.width, defaultSize.height, defaultSettings));
    const frame = useRef(null);
    const lastTime = useRef(null);
    const loadedSharedScene = useRef(false);
    const glucoseStageTimers = useRef([]);
    const clearGlucoseStageTimers = useCallback(() => {
        for (const timer of glucoseStageTimers.current)
    ...
    const spawnAtom = useCallback((symbol) => {
    ...
    const loadPreset = useCallback((presetId, nextMode = "presets") => {
        ...
        if (isGlucoseRingPreset(preset.id))
            setGlucoseAnomer(preset.id === "glucose-alpha" ? "alpha" : "beta");
        if (nextMode === "guided") {
    ...
    const loadMoleculePreset = useCallback((preset, nextMode = "presets") => {
        clearGlucoseStageTimers();
        setMode(nextMode);
        setActivePreset(preset);
        setGlucoseStage(preset.id === "glucose-linear" ? "aldehyde" : isGlucoseRingPreset(preset.id) ? "ring" : "idle");
    ...
    useEffect(() => {
        ...
        if (!match)
            return;
        try {
            const payload = JSON.parse(decodeURIComponent(escape(window.atob(match[1]))));
            if (payload.settings) {
    ...
    const selectAtom = useCallback((atomId) => {
        setState((current) => ({ ...current, selectedAtomId: atomId, selectedBondId: null }));
    }, []);
    const selectBond = useCallback((bondId) => {
        setState((current) => ({ ...current, selectedBondId: bondId, selectedAtomId: null }));
    }, []);
    const moveAtom = useCallback((atomId, x, y) => {
        setState((current) => {
            const movedAtoms = current.atoms.map((atom) => (atom.id === atomId
            ...
            const remainingBondIdsByAtom = new Map();
            ...
            const atoms = movedAtoms.map((atom) => {
                ...
                if (!affectedAtomIds.has(atom.id))
                    return { ...atom, bonds };
                return {
                    ...atom,
                    bonds,
    ...
    const formGlucoseRing = useCallback((anomer = glucoseAnomer) => {
    ...
    const setGlucoseAnomerTarget = useCallback((anomer) => {
        setGlucoseAnomer(anomer);
        if (activePreset?.id === "glucose-linear") {
            setGlucoseStage("aldehyde");
            return;
        }
        if (!isGlucoseRingPreset(activePreset?.id ?? ""))
            return;
        const ring = glucoseRingPresetForAnomer(anomer);
        clearGlucoseStageTimers();
        setActivePreset(ring);
    ...
    const selectedAtom = useMemo(() => (state.atoms.find((atom) => atom.id === state.selectedAtomId) ?? null), [state.atoms, state.selectedAtomId]);
    const selectedBond = useMemo(() => (state.bonds.find((bond) => bond.id === state.selectedBondId) ?? null), [state.bonds, state.selectedBondId]);
    const selectedMolecule = useMemo(() => {
        if (!selectedAtom)
            return [];
        const seen = new Set([selectedAtom.id]);
        let changed = true;
        while (changed) {
            changed = false;
    ...
    const setLessonState = useCallback((annotations, highlightAtoms, highlightBonds, stepIdx, stepTxt, total, parts, reveal) => {
    ...
    const addLessonAtom = useCallback((symbol, x, y) => {
    ...
    const moveLessonAtom = useCallback((atomIndex, targetX, targetY) => {
        const offset = lessonSceneOffset(size.width, size.height);
        setState((current) => {
            const atoms = current.atoms.map((a, i) => i === atomIndex ? { ...a, targetX: targetX + offset.x, targetY: targetY + offset.y, guided: true } : a);
            return { ...current, atoms };
        });
    }, [size.width, size.height]);
    const bondLessonAtoms = useCallback((atomIndexA, atomIndexB) => {
        setState((current) => {
            ...
            if (!a || !b)
                return current;
            if (current.bonds.some((item) => (item.a === a.id && item.b === b.id) || (item.a === b.id && item.b === a.id)))
                return current;
    ...
    const addLessonParticles = useCallback((fromIndex, toIndex, count, color) => {
        setState((current) => {
    ...
    const flingAtom = useCallback((atomId, vx, vy) => {
        setState((current) => {
            const atoms = current.atoms.map((atom) => atom.id === atomId ? { ...atom, vx, vy } : atom);
            return { ...current, atoms };
        });
    }, []);
...
function createPresetBondsForAtoms(preset, atoms, time) {
...
function attachBondIds(atoms, bonds) {
    const idsByAtom = new Map();
...
function isGlucoseRingPreset(id) {
    return id === "glucose-ring" || id === "glucose-alpha" || id === "glucose-beta";
}
function glucoseRingPresetForAnomer(anomer) {
...
function glucoseHemiacetalPresetForAnomer(anomer) {
    const linear = presetById["glucose-linear"];
    const ring = glucoseRingPresetForAnomer(anomer);
...
function retargetPresetState(current, preset, fallback, width, height, time, options = {}) {
