import { atomData } from "./atoms.js";
import { classifyBond } from "../simulation/chemistry.js";
...
export async function fetchPubChemMolecule(query) {
    const cleanQuery = query.trim();
    if (!cleanQuery)
        throw new Error("Type a molecule name first.");
    ...
    return {
        id: `pubchem-${cid}`,
        name: title,
        aliases: [cleanQuery, properties.IUPACName, properties.CanonicalSMILES, properties.IsomericSMILES].filter(Boolean),
        formula,
        category: "advanced",
...
export async function fetchPubChemSuggestions(query, signal) {
    const cleanQuery = query.trim();
...
async function fetchFirstCid(query) {
    const url = `${PUBCHEM_BASE}/compound/name/${encodeURIComponent(query)}/cids/JSON`;
    const data = await fetchJson(url);
    const cid = data.IdentifierList?.CID?.[0];
    if (!cid)
        throw new Error("PubChem did not find that molecule name.");
    return cid;
}
...
async function fetchSdf(cid) {
    ...
    if (!response.ok)
        throw new Error(`PubChem SDF request failed with ${response.status}.`);
...
function parseSdf(sdf) {
    ...
    if (!counts)
        throw new Error("PubChem returned an unreadable structure.");
    ...
    const atoms = normalizeCoordinates(rawAtoms.map((atom) => ({
        ...atom,
        symbol: atom.symbol
    })), rawBonds);
    for (const bond of rawBonds) {
        const kind = classifyBond(atoms[bond.a].symbol, atoms[bond.b].symbol, 1);
        bonds.push({ ...bond, kind });
    }
    return { atoms, bonds, unsupported: [...unsupported].sort() };
}
function normalizeCoordinates(atoms, bonds) {
    if (!atoms.length)
        return atoms;
