import { buildMoleculeGraph, structuralBonds } from "./graph.js";
import { analyzeAtomGeometry } from "./vsepr.js";
const clamp = (value, min, max) => Math.min(max, Math.max(min, value));
export function applyMolecularRelaxation(atoms, bonds, settings, dt) {
    if (!settings.geometryAssist || atoms.length < 2)
        return;
    ...
    if (settings.geometry3D)
        applyTargetDepthSprings(atoms, strength, dt);
...
function applyBondSprings(atoms, bonds, strength, dt) {
...
function applyTargetDepthSprings(atoms, strength, dt) {
    for (const atom of atoms) {
        if (atom.targetZ === undefined)
            continue;
...
function applyVseprAngleForces(atoms, bonds, strength, dt) {
    ...
    for (const center of atoms) {
        ...
        if (!analysis || neighbors.length < 2 || analysis.relaxAngle <= 0)
            continue;
...
function applyNonbondedRepulsion(atoms, bonds, settings, strength, dt) {
