import { structuralBonds } from "./graph.js";
const groupColor = {
    alcohol: "#06b6d4",
    amine: "#8b5cf6",
    carbonyl: "#f43f5e",
...
function makeGroup(kind, label, atomIds, bondIds) {
    return {
...
function findAromaticRings(atoms, bonds, neighbors) {
...
function walkCarbonCycle(start, current, path, bondPath, neighbors, cycles) {
    const nextPath = [...path, current];
    if (nextPath.length > 6)
        return;
    for (const next of neighbors.get(current) ?? []) {
        if (next.id === start && nextPath.length === 6) {
            ...
            if (!cycles.has(key))
                cycles.set(key, { atomIds, bondIds: [...bondPath, next.bond.id] });
            continue;
        }
        if (nextPath.includes(next.id))
            continue;
        if (next.id < start)
            continue;
