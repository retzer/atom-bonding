...
import { applyIonCharges, canBond, classifyBond, eventForBond, makeBond } from "./chemistry.js";
import { applyMolecularRelaxation } from "./relaxation.js";
import { applyVsepr3DTargets } from "./vsepr3d.js";
const rand = (min, max) => min + Math.random() * (max - min);
const clamp = (value, min, max) => Math.min(max, Math.max(min, value));
const idPart = () => `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 9)}`;
export function createAtom(symbol, x, y, guided = false) {
    const atom = atomData[symbol];
    if (!atom)
        throw new Error(`Unsupported atom symbol: ${symbol}`);
    return {
        id: `${symbol}-${idPart()}`,
        symbol,
        x,
...
export function createFreeState(width, height, settings) {
    const atoms = [];
    return {
        atoms,
        bonds: [],
...
export function createPresetState(width, height, preset) {
    ...
    const metallicElectrons = preset.metallic
...
export function stepSimulation(state, settings, width, height, rawDt) {
    ...
    const atoms = state.atoms.map((atom) => ({ ...atom, bonds: [] }));
    ...
    const metallicElectrons = state.metallicElectrons.map((electron) => {
        ...
        next.y += next.vy * dt;
        const pad = 58;
        if (next.x < worldBounds.minX + pad || next.x > worldBounds.maxX - pad)
            next.vx *= -1;
        if (next.y < worldBounds.minY + pad || next.y > worldBounds.maxY - pad)
            next.vy *= -1;
        return next;
    });
...
function findHydrogenBonds(atoms, bonds) {
    ...
    for (const hydrogen of atoms.filter((atom) => atom.symbol === "H")) {
        ...
        if (!donor || !["O", "N", "F"].includes(donor.symbol))
            continue;
        for (const acceptor of acceptors) {
            if (acceptor.id === donor.id)
                continue;
            if (structuralBonds.some((bond) => (bond.a === hydrogen.id && bond.b === acceptor.id) || (bond.b === hydrogen.id && bond.a === acceptor.id)))
                continue;
