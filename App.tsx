    ...
    <main className={`app-shell ${sim.settings.theme}`}>
      ...
      <section className="workspace">
        <div className="left-column">
          ...
          />
          <ControlDock settings={sim.settings} onSetting={sim.updateSetting} onSpawnAtoms={() => {
            sim.setMode("free");
            sim.spawnFreeAtoms();
          }} />
        </div>
        <div className="right-column">
          <LearningPanel mode={sim.mode} activePresetId={sim.activePreset?.id} onPreset={(id) => sim.loadPreset(id, sim.mode === "guided" ? "guided" : "presets")} />
          <InfoPanel
